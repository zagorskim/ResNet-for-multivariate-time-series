...
read_data <- function() {
  ...
  print(datasets)
  cat("Сhoose dataset to analyse from visible above.\n")
  chosenDataset <- readline("")
  ...
  correct <- FALSE
  while (correct == FALSE) {
    cat("Сhoose file format: (type \"txt\" or \"arff\")\n")
    fileFormat <- readline("")
    if (fileFormat == "arff") {
      connection <-
        file(description = paste("./data/", chosenDataset, "_TRAIN.arff", sep = ""))
      dfTrain <- read.arff(connection)
      connection <-
        file(description = paste("./data/", chosenDataset, "_TEST.arff", sep = ""))
      dfTest <- read.arff(connection)
      correct = TRUE
    } else if (fileFormat == "txt")
    {
      connection <-
        file(description = paste("./data/", chosenDataset, "_TRAIN.txt", sep = ""))
      dfTrain <- read.table(connection)
      connection <-
        file(description = paste("./data/", chosenDataset, "_TEST.txt", sep = ""))
...
cut_preprocessing <- function (df, isTxt) {
  ...
  classes <- split(df, df[index])
  splittingFactor <-
    lapply(classes, function(x)
      sample(factor(sort(1:length(
        x[[1]]
      )) %% 3)))
  splitList <- list()
  for (index in 1:length(classes)) {
    splitList[[index]] <-
      split(classes[[index]], splittingFactor[[index]])
  }
  cut <- lapply(splitList, function(x)
    cut_series(x, isTxt))
  for (index in 1:length(classes)) {
    classes[[index]] <- unsplit(cut[[index]], splittingFactor[[index]])
  }
  df <- unsplit(classes, df[index])
  
  return(df)
}

cut_store <- function(dfTrain, dfTest, isTxt) {
  # Cutting
  train <- cut_preprocessing(dfTrain, isTxt)
  test <- cut_preprocessing(dfTest, isTxt)
  
  # Cut output settings and process
  # TBD Mark target or do sth else with it
  cat(
    "Enter directory to save cut data ({path}/{filename_without_extension}. (press enter to set default {working_directory}/cut_data/ )\n"
  )
  chosenDir <- readline("")
  if (chosenDir == "")
  ...
  correct = FALSE
  while (correct == FALSE) {
    cat("Choose saving file format. (type \"txt\" or \"arff\")\n")
    chosenSaveFormat <- readline("")
...
get_data <- function() {
  ans <-
    take_input("Do you want to download new data?", c("yes", "no"))
  if (ans == "yes") {
    to[dfTrain <-
         dfTrain, dfTest <- dfTest, isTxt <- isTxt] <- read_data()
    temp <- cut_store(dfTrain, dfTest, isTxt)
    train <- temp[[1]]
    test <- temp[[2]]
  } else {
    ans <- take_input("Enter path for train data.")
    connection <- file(description = ans)
    if (substr(ans, nchar(ans) - 3, nchar(ans)) == "arff") {
      train <- as.list(read.arff(connection))
    } else if (substr(ans, nchar(ans) - 2, nchar(ans)) == "txt")
    {
      train <- as.list(read.table(connection))
    }
    ans <- take_input("Enter path for test data.")
    connection <- file(description = ans)
    if (substr(ans, nchar(ans) - 3, nchar(ans)) == "arff") {
  ...
  #   na.omit(x))
  return(list(trainClear, testClear))
}

take_input <- function(prompt, options = NULL) {
  ans <- "-1"
  if (length(options) > 0)
    while (!(ans %in% options)) {
      cat(paste(prompt, " (", toString(options), ")", "\n", sep = ""))
      ans <- readline("")
    }
  else
  {
